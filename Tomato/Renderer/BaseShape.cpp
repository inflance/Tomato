...
namespace Tomato
{
	void BaseShape::CreateCube(int entityID)
	{
		...
		mVertexBuffer = VertexBuffer::Create(sizeof(Vertex) * m_base_shape.Vertices.size());
		...
		m_vertex_array->AddVertexBuffer(mVertexBuffer);

		/*mIndexBuffer = IndexBuffer::Create(m_base_shape.indices.data(), m_base_shape.indices.size());
		m_vertex_array->SetIndexBuffer(mIndexBuffer);*/
	}
	...
	{
		mVertexBuffer->SetData(m_base_shape.Vertices.data(), m_base_shape.Vertices.size() * sizeof(Vertex));
	}
}
