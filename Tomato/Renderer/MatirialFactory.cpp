...
namespace Tomato {
	...
	Tomato::MatirialTextureData& MatirialFactory::Load(const std::string& path, MatirialTextureData& data)
	{
	...
	Tomato::MatirialTextureData& MatirialFactory::GetMatirial(const std::string& path)
	{
		LOG_ASSERT(Exist(path), "");
		return m_matirial_factory[path];
	}
	...
	{
		return m_matirial_factory.find(path) != m_matirial_factory.end();
	}
