	...
	void Mesh::GetStaticMeshData(aiMesh* mesh, const aiScene* scene)
	{
		...
		for (unsigned int i = 0; i < mesh->mNumVertices; i++)
		{
			...
			if (mesh->HasNormals())
			{
				...
				LogSystem::Log(LogType::Warn, "Mesh don`t have normal");
			}
			// texture coordinates
		...
		std::vector<MatirialTextureData> diffuseMaps = LoadMaterialTextures(material, aiTextureType_DIFFUSE, PBRTextureType::Diffuse);
		textures.insert(textures.end(), diffuseMaps.begin(), diffuseMaps.end());
		// 2. specular maps
		std::vector<MatirialTextureData> specularMaps = LoadMaterialTextures(material, aiTextureType_SPECULAR, PBRTextureType::Specular);
		textures.insert(textures.end(), specularMaps.begin(), specularMaps.end());
		// 3. normal maps
		std::vector<MatirialTextureData> normalMaps = LoadMaterialTextures(material, aiTextureType_HEIGHT, PBRTextureType::Normal);
		textures.insert(textures.end(), normalMaps.begin(), normalMaps.end());
		// 4. height maps
		std::vector<MatirialTextureData> heightMaps = LoadMaterialTextures(material, aiTextureType_AMBIENT, PBRTextureType::Ambient);
		textures.insert(textures.end(), heightMaps.begin(), heightMaps.end());
		m_submeshs.emplace_back(SubMesh(veries, indices, textures));
	}

	std::vector<MatirialTextureData> Mesh::LoadMaterialTextures(aiMaterial* mat, aiTextureType type, PBRTextureType typeName)
	{
