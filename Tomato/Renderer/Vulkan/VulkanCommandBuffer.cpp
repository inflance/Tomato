...
namespace Tomato
{
	VulkanCommandBuffer::VulkanCommandBuffer(uint32_t count, std::string debugName)
		: m_DebugName(std::move(debugName))
	{
		auto device = VulkanContext::GetCurrentDevice();
		uint32_t framesInFlight = Renderer::s_renderer_config.max_frame;

		VkCommandPoolCreateInfo cmdPoolInfo = {};
		cmdPoolInfo.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO;
		cmdPoolInfo.queueFamilyIndex = device->GetPhysicalDevice()->GetQueueFamilyIndices().Graphics.value();
		cmdPoolInfo.flags = VK_COMMAND_POOL_CREATE_TRANSIENT_BIT | VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT;
		VK_CHECK_RESULT(vkCreateCommandPool(device->GetVulkanDevice(), &cmdPoolInfo, nullptr, &m_CommandPool), "");
		Utils::SetDebugUtilsObjectName(device->GetVulkanDevice(), VK_OBJECT_TYPE_COMMAND_POOL, m_DebugName,
		                                 m_CommandPool);
		...
		VK_CHECK_RESULT(
			vkAllocateCommandBuffers(device->GetVulkanDevice(), &commandBufferAllocateInfo, m_CommandBuffers.data()), "");

		for (uint32_t i = 0; i < count; ++i)
			Utils::SetDebugUtilsObjectName(device->GetVulkanDevice(), VK_OBJECT_TYPE_COMMAND_BUFFER,
			                                 fmt::format("{} (frame in flight: {})", m_DebugName, i),
			                                 m_CommandBuffers[i]);
		...
		for (size_t i = 0; i < m_WaitFences.size(); ++i)
		{
			...
			Utils::SetDebugUtilsObjectName(device->GetVulkanDevice(), VK_OBJECT_TYPE_FENCE,
			                                 fmt::format("{} (frame in flight: {}) fence", m_DebugName, i),
			                                 m_WaitFences[i]);
		}
		...
		for (auto& timestampQueryPool : m_TimestampQueryPools)
			VK_CHECK_RESULT(
			vkCreateQueryPool(device->GetVulkanDevice(), &queryPoolCreateInfo, nullptr, &timestampQueryPool),"");
		...
		for (auto& pipelineStatisticsQueryPools : m_PipelineStatisticsQueryPools)
			VK_CHECK_RESULT(
			vkCreateQueryPool(device->GetVulkanDevice(), &queryPoolCreateInfo, nullptr, &pipelineStatisticsQueryPools
			), "");

		m_PipelineStatisticsQueryResults.resize(framesInFlight);
	}

	VulkanCommandBuffer::VulkanCommandBuffer(std::string debugName, bool swapchain)
	...
	VulkanCommandBuffer::~VulkanCommandBuffer()
	{
		...
		Renderer::SubmitAndFree([commandPool]()
		{
			auto device = VulkanContext::GetCurrentDevice();
	...
	void VulkanCommandBuffer::Begin()
	{
		...
		VulkanCommandBuffer* instance = this;
		Renderer::Submit([instance]() mutable
		{
				auto swapChain = VulkanContext::GetCurrentSwapChain();
		uint32_t frameIndex = swapChain->GetCurrentBufferIndex();
			...
			// Timestamp query
			vkCmdResetQueryPool(commandBuffer, instance->m_TimestampQueryPools[frameIndex], 0,
			                    instance->m_TimestampQueryCount);
			vkCmdWriteTimestamp(commandBuffer, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT,
			                    instance->m_TimestampQueryPools[frameIndex], 0);

			// Pipeline stats query
			vkCmdResetQueryPool(commandBuffer, instance->m_PipelineStatisticsQueryPools[frameIndex], 0,
			                    instance->m_PipelineQueryCount);
	...
	void VulkanCommandBuffer::End()
	{
		...
		uint32_t frameIndex = swapChain->GetCurrentBufferIndex();
			...
			vkCmdWriteTimestamp(commandBuffer, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT,
			                    instance->m_TimestampQueryPools[frameIndex], 1);
	...
	void VulkanCommandBuffer::Submit()
	{
		...
		Renderer::Submit([instance]() mutable
		{
			...
			VK_CHECK_RESULT(
				vkWaitForFences(device->GetVulkanDevice(), 1, &instance->m_WaitFences[frameIndex], VK_TRUE, UINT64_MAX
				),"");
			VK_CHECK_RESULT(vkResetFences(device->GetVulkanDevice(), 1, &instance->m_WaitFences[frameIndex]), "");
			VK_CHECK_RESULT(
				vkQueueSubmit(device->GetGraphicsQueue(), 1, &submitInfo, instance->m_WaitFences[frameIndex]), "");

			// Retrieve timestamp query results
			vkGetQueryPoolResults(device->GetVulkanDevice(), instance->m_TimestampQueryPools[frameIndex], 0,
			                      instance->m_TimestampNextAvailableQuery,
			                      instance->m_TimestampNextAvailableQuery * sizeof(uint64_t),
			                      instance->m_TimestampQueryResults[frameIndex].data(), sizeof(uint64_t),
			...
			for (uint32_t i = 0; i < instance->m_TimestampNextAvailableQuery; i += 2)
			{
				...
				float nsTime = endTime > startTime
					               ? (endTime - startTime) * device->GetPhysicalDevice()->GetLimits().timestampPeriod
					               : 0.0f;
			...
			vkGetQueryPoolResults(device->GetVulkanDevice(), instance->m_PipelineStatisticsQueryPools[frameIndex], 0, 1,
			                      sizeof(PipelineStatistics), &instance->m_PipelineStatisticsQueryResults[frameIndex],
			                      sizeof(uint64_t), VK_QUERY_RESULT_64_BIT);
		});
	}

	uint32_t VulkanCommandBuffer::BeginTimestampQuery()
	{
		...
		uint32_t frameIndex = swapChain->GetCurrentBufferIndex();
			VkCommandBuffer commandBuffer = instance->m_CommandBuffers[frameIndex];
			vkCmdWriteTimestamp(commandBuffer, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT,
			                    instance->m_TimestampQueryPools[frameIndex], queryIndex);
		});
		return queryIndex;
	}

	void VulkanCommandBuffer::EndTimestampQuery(uint32_t queryID)
	{
		VulkanCommandBuffer* instance = this;
		...
		uint32_t frameIndex = swapChain->GetCurrentBufferIndex();
			VkCommandBuffer commandBuffer = instance->m_CommandBuffers[frameIndex];
			vkCmdWriteTimestamp(commandBuffer, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT,
			                    instance->m_TimestampQueryPools[frameIndex], queryID + 1);
		});
	}
}
