...
namespace Tomato
{
	...
	void VulkanPhysicalDevice::Init()
	{
		...
		{
			LOG_TRACE("Could not find discrete GPU.");
			selectedPhysicalDevice = physicalDevices.back();
		}

		LOG_ASSERT(selectedPhysicalDevice, "Could not find any physical devices!");
		...
		if (requestedQueueTypes & VK_QUEUE_GRAPHICS_BIT)
		{
			...
			queueInfo.queueFamilyIndex = m_QueueFamilyIndices.Graphics.value();
		...
		if (requestedQueueTypes & VK_QUEUE_COMPUTE_BIT)
		{
			if (m_QueueFamilyIndices.Compute != m_QueueFamilyIndices.Graphics)
			{
				...
				VkDeviceQueueCreateInfo queueInfo{};
				queueInfo.sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO;
				queueInfo.queueFamilyIndex = m_QueueFamilyIndices.Compute.value();
				queueInfo.queueCount = 1;
				queueInfo.pQueuePriorities = &defaultQueuePriority;
				m_QueueCreateInfos.push_back(queueInfo);
			}
		...
		if (requestedQueueTypes & VK_QUEUE_TRANSFER_BIT)
		{
			if ((m_QueueFamilyIndices.Transfer != m_QueueFamilyIndices.Graphics) && (m_QueueFamilyIndices.Transfer != m_QueueFamilyIndices.Compute))
			{
				...
				VkDeviceQueueCreateInfo queueInfo{};
				queueInfo.sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO;
				queueInfo.queueFamilyIndex = m_QueueFamilyIndices.Transfer.value();
				queueInfo.queueCount = 1;
				queueInfo.pQueuePriorities = &defaultQueuePriority;
				m_QueueCreateInfos.push_back(queueInfo);
			}
		...
		LOG_ASSERT(m_DepthFormat,"");
	}

	void VulkanPhysicalDevice::Destroy() const
	{
		LOG_INFO("Vulkan: Vulkan Physical Device Destroy!");
	}

	uint32_t VulkanPhysicalDevice::GetMemoryType(uint32_t type_bits, VkMemoryPropertyFlags properties) const
	{
		...
		LOG_ASSERT(false, "Could not find a suitable memory type!");
	...
	QueueFamilyIndices VulkanPhysicalDevice::FindQueueFamilies(int flags)
	{
		...
		if (flags & VK_QUEUE_COMPUTE_BIT)
		{
			for (uint32_t i = 0; i < m_QueueFamilyProperties.size(); i++)
			{
				...
				if ((queueFamilyProperties.queueFlags & VK_QUEUE_COMPUTE_BIT) && ((queueFamilyProperties.queueFlags &
		...
		if (flags & VK_QUEUE_TRANSFER_BIT)
		{
			...
			{
				auto& queueFamilyProperties = m_QueueFamilyProperties[i];
				if ((queueFamilyProperties.queueFlags & VK_QUEUE_TRANSFER_BIT) && ((queueFamilyProperties.queueFlags &
					VK_QUEUE_GRAPHICS_BIT) == 0) && ((queueFamilyProperties.queueFlags & VK_QUEUE_COMPUTE_BIT) == 0))
				{
					indices.Transfer = i;
					break;
				}
		...
		: m_PhysicalDevice(physicalDevice), m_EnabledFeatures(enabledFeatures)
	{
		...
		LOG_ASSERT(m_PhysicalDevice->IsExtensionSupported(VK_KHR_SWAPCHAIN_EXTENSION_NAME),"");
		...
		VkResult result = vkCreateDevice(m_PhysicalDevice->GetVkPhysicalDevice(), &deviceCreateInfo, nullptr, &m_LogicalDevice);
	...
	void VulkanDevice::FlushCommandBuffer(VkCommandBuffer commandBuffer, VkQueue queue)
	{
		...
		LOG_ASSERT(commandBuffer != VK_NULL_HANDLE,"");
	...
	VkCommandBuffer VulkanDevice::CreateSecondaryCommandBuffer(const char* debugName) const
	{
		VkCommandBuffer cmdBuffer;
		...
		VK_CHECK_RESULT(vkAllocateCommandBuffers(m_LogicalDevice, &cmdBufAllocateInfo, &cmdBuffer),"");
		Utils::SetDebugUtilsObjectName(m_LogicalDevice, VK_OBJECT_TYPE_COMMAND_BUFFER, debugName, cmdBuffer);
		return cmdBuffer;
	}
}
