...
namespace Tomato
{
	...
	void VulkanSwapChain::InitSurface(GLFWwindow* windowHandle)
	{
		...
		uint32_t graphicsQueueNodeIndex = Max<uint32_t>();
		uint32_t presentQueueNodeIndex = Max<uint32_t>();
		for (uint32_t i = 0; i < queueCount; i++)
		{
			if ((queueProps[i].queueFlags & VK_QUEUE_GRAPHICS_BIT) != 0)
			{
				if (graphicsQueueNodeIndex == Max<uint32_t>())
				{
		...
		if (presentQueueNodeIndex == Max<uint32_t>())
		{
		...
		ASSERT(graphicsQueueNodeIndex != Max<uint32_t>());
		ASSERT(presentQueueNodeIndex != Max<uint32_t>());
	...
	{
		LOG_WARN("Renderer", "VulkanSwapChain::OnDestroy");
	...
	{
		LOG_WARN("Renderer", "VulkanSwapChain::OnResize");
	...
	{
		LOG_WARN("VulkanSwapChain::BeginFrame");
	...
	{
		LOG_WARN("VulkanSwapChain::Present");
		...
		VkResult result;
		{
			LOG_WARN("VulkanSwapChain::Present - QueuePresent");
		...
		{
			LOG_WARN("VulkanSwapChain::Present - WaitForFences");
			const auto& config = Renderer::s_renderer_config;
			m_CurrentBufferIndex = (m_CurrentBufferIndex + 1) % config.max_frame;
			// Make sure the frame we're requesting has finished rendering
			VK_CHECK_RESULT(vkWaitForFences(m_Device->GetVulkanDevice(), 1, &m_WaitFences[m_CurrentBufferIndex], VK_TRUE, Max<uint64_t>()),"");
		}
	}
